...
> 检索截止：2026-08-15。本文的“测试”对象是会规划、调用工具、改变环境或与其他 Agent 协作的软件智能体系统，而不是普通单轮 LLM 问答。详细证据见 [`software-agent-testing-evidence-2026.csv`](../../literature/evidence/software-agent-testing-evidence-2026.csv)，检索记录见 [`software-agent-testing-search-log-2026.md`](../../searches/logs/software-agent-testing-search-log-2026.md)。
...
本文使用仓库 [`references.bib`](../../literature/bibliography/references.bib) 中的 BibTeX 引文键。证据表保存每篇论文的 DOI/arXiv ID、来源覆盖、版本关系、证据等级、主要限制和核验状态。无 DOI 预印本不会因 Crossref 没有记录而被写成“无效”；它们会被标为 arXiv 身份已核对、正式版本未核验。
