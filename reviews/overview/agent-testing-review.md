...
> **更新提示（2026-08-13）：** 新增动态测试生成、灰盒模糊测试、agentic oracle、轨迹回归及 USENIX Security 2026 安全测试进展，见 `reviews/overview/2026-08-13-agent-testing-update.md`。本文原有 40 篇核心语料与统计口径保持不变。

> 检索截止：2026-07-23。正文中的方括号为 `literature/bibliography/references.bib` 的 BibTeX 引文键。
> 本综述纳入 40 篇身份已核验论文，其中 25 篇完成全文精读并有独立结构化笔记，15 篇仅在摘要允许的范围内使用。完整纳排记录见 `literature/evidence/screening-log.csv`，逐篇证据等级见 `literature/evidence/evidence-table.csv`。
...
五个完整可实施选题、实验设计和简历成果另见 `reviews/overview/research-directions.md`。

2026 年最新预印本与观察性进展另见 `reviews/overview/2026-latest-update.md`。该专题单独标记摘要级证据，不与主体 40 篇核心语料混合排名。
...
完整 BibTeX 位于 `literature/bibliography/references.bib`，包括主体 40 条正式引文和 12 条独立标记的 2026 watchlist。下表给出本综述 40 条核心引文的身份与证据边界；DOI 只在 Crossref/OpenAlex或正式出版页核验后写入 BibTeX。
...
这批证据强化了四点结论：注错实验必须验证故障是否实际触发；最终成功率之外应报告传播、恢复和定位；LLM Judge 的叙述不能等同因果证据；trace 可见度本身是评测条件，并与隐私形成直接权衡。详细方法表、数值来源、版本冲突和研究空白见 `reviews/overview/2026-08-14-agent-testing-methods-fulltext-review.md`。
